...
export const embedRoutes = async (app) => {
    ...
    app.post('/bots/:botId/chat', async (request, reply) => {
        const { botId } = request.params;
        const { message, conversationId, sessionId } = request.body;
        ...
        try {
            ...
            app.log.error({ err: error }, 'OpenAI error');
        ...
        if (conv.visitorId && conv.messages.length > 3) {
            const leadInfo = await app.ai.extractLeadInfo([...conv.messages, { role: 'user', content: message }, { role: 'assistant', content: assistantMessage.content }], bot.model || 'gpt-4-turbo-preview');
    ...
    app.post('/bots/:botId/leads', async (request, reply) => {
        const { botId } = request.params;
        const { conversationId, name, email, phone, company, metadata } = request.body;
        if (!name || !email) {
