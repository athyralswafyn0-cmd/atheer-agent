...
export const partnerRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/organizations/:organizationId/usage', async (request, _reply) => {
    const { organizationId } = request.params as { organizationId: string };
    const { startDate, endDate, page = 1, limit = 30 } = request.query as {
      startDate?: string;
      endDate?: string;
      page?: number;
      limit?: number;
    ...
    const where: Prisma.UsageWhereInput = { organizationId };
  ...
  app.get('/:partnerId/usage', async (request, _reply) => {
    ...
    const where: Prisma.UsageWhereInput = { organizationId: { in: orgIds } };
    if (startDate || endDate) {
      where.date = {};
      if (startDate) where.date.gte = new Date(startDate);
      if (endDate) where.date.lte = new Date(endDate);
