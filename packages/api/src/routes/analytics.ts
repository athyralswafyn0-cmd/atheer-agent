...
export const analyticsRoutes: FastifyPluginAsync = async (app) => {
  ...
  const getFromDate = (param: string | string[] | undefined): Date => {
    if (Array.isArray(param)) {
      param = param[0];
    }
    return typeof param === 'string' ? new Date(param) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  };
  const getToDate = (param: string | string[] | undefined): Date => {
    if (Array.isArray(param)) {
      param = param[0];
    }
  ...
  app.get('/bot/:botId/overview', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const query = request.query as { [key: string]: string | string[] | undefined };
    ...
    const user = request.user as { userId: string; organizationId: string; role?: string };
    ...
    reply.send({
      overview: {
        ...
        totalMessages,
        totalLeads,
        avgResponseTime: (avgResponseTime as any)?.[0]?.avg_seconds || 0,
        satisfactionRate: (satisfactionRate as any)?.[0]?.avg_rating || 0,
      ...
      leads: {
        byStatus: leadsByStatus.map((l: any) => ({ status: l.status, count: l._count })),
      },
      satisfaction: {
        distribution: (satisfactionDistribution as any).map((d: any) => ({ rating: d.rating, count: Number(d.count) })),
      },
      responseTime: {
        percentiles: (responseTimePercentiles as any)?.[0] || { p50: 0, p90: 0, p95: 0, p99: 0 },
      },
      languages: topLanguages.map((l: any) => ({ language: l.language, count: l._count })),
      period: { from: dateFrom.toISOString(), to: dateTo.toISOString() },
    });
  });

  app.get('/bot/:botId/conversations', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const { from, to, interval = 'day' } = request.query as { from?: string; to?: string; interval?: 'hour' | 'day' | 'week' };
  ...
  app.get('/bot/:botId/satisfaction', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    ...
    reply.send({
      distribution: (distribution as any).map((d: any) => ({ rating: d.rating, count: Number(d.count) })),
      trend: (trend as any).map((d: any) => ({ day: d.day, avgRating: d.avg_rating })),
    });
  });

  app.get('/bot/:botId/response-time', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const { from, to } = request.query as { from?: string; to?: string };
    ...
    reply.send({
      percentiles: (percentiles as any)?.[0] || { p50: 0, p75: 0, p90: 0, p95: 0, p99: 0 },
      trend: (trend as any).map((d: any) => ({ day: d.day, avgResponseTime: d.avg_response })),
    });
  });

  app.get('/bot/:botId/leads', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const { from, to } = request.query as { from?: string; to?: string };
    ...
    reply.send({
      funnel: funnel.map((f: any) => ({ status: f.status, count: f._count })),
      bySource: bySource.map((s: any) => ({ source: s.source, count: s._count })),
      byDay: (byDay as any).map((d: any) => ({ day: d.day, count: Number(d.count) })),
      topBots: topBots.map((b: any) => ({ botId: b.botId, count: b._count })),
    });
  });

  app.get('/bot/:botId/top-questions', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const { from, to, limit = 20 } = request.query as { from?: string; to?: string; limit?: number };
  ...
  app.get('/bot/:botId/export', async (request: FastifyRequest, reply: FastifyReply) => {
    const { botId } = request.params as { botId: string };
    const { type = 'conversations', format = 'json', from, to } = request.query as {
      type?: 'conversations' | 'leads' | 'messages';
      format?: 'json' | 'csv';
      from?: string;
      to?: string;
    };
