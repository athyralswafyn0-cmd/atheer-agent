...
const authModule = {
  ...
  async logout(userId: string) { return { success: true }; },
  async requestPasswordReset(email: string) { return { message: 'If your email is registered, you will receive a password reset link' }; },
  async resetPassword(token: string, password: string) {
    const resetToken = await prisma.passwordResetToken.findUnique({ where: { token } });
    if (!resetToken || resetToken.expires < new Date()) throw new Error('INVALID_OR_EXPIRED_TOKEN');
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    ...
    if (!user || !user.passwordHash) throw new Error('CURRENT_PASSWORD_INCORRECT');
  ...
  async revokeSession(userId: string, sessionId: string) { return { success: true }; },
  async revokeAllSessions(userId: string) { return { success: true }; },
};
...
app.post('/api/v1/auth/register', async (request, reply) => {
  try {
    ...
    if (error.message === 'EMAIL_ALREADY_REGISTERED') return reply.code(409).send({ error: 'Email already registered' });
    throw error;
  }
});
...
app.post('/api/v1/auth/login', async (request, reply) => {
  try {
    ...
    if (error.message === 'INVALID_CREDENTIALS') return reply.code(401).send({ error: 'Invalid credentials' });
    throw error;
  }
});
...
app.post('/api/v1/auth/refresh', async (request, reply) => {
  ...
  } catch { return reply.code(401).send({ error: 'Invalid or expired refresh token' }); }
});
...
app.post('/api/v1/auth/reset-password', async (request, reply) => {
  try {
    ...
    if (error.message === 'INVALID_OR_EXPIRED_TOKEN') return reply.code(400).send({ error: 'Invalid or expired token' });
    throw error;
  }
});
...
app.post('/api/v1/auth/change-password', { preHandler: [app.authenticate] }, async (request, reply) => {
  try {
    ...
    if (error.message === 'CURRENT_PASSWORD_INCORRECT') return reply.code(400).send({ error: 'Current password is incorrect' });
    throw error;
  }
});
...
app.post('/api/v1/organizations', { preHandler: [app.authenticate] }, async (request, reply) => {
  const user = request.user!;
  if (!['PARTNER_ADMIN', 'OWNER', 'ADMIN'].includes(user.role)) return reply.code(403).send({ error: 'Insufficient permissions' });
  const body = request.body as { name: string; slug?: string; [key: string]: any };
