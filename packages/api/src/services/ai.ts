...
export class AIService {
  ...
  async createEmbedding(text: string, model = 'text-embedding-3-large') {
  ...
  async createEmbeddingsBatch(texts: string[], model = 'text-embedding-3-large') {
    const response = await this.openai.embeddings.create({
      model,
      input: texts,
      encoding_format: 'float',
  ...
  async summarizeConversation(messages: Array<{ role: string; content: string }>, model = 'gpt-4-turbo-preview') {
  ...
  async extractLeadInfo(messages: Array<{ role: string; content: string }>, model = 'gpt-4-turbo-preview') {
    const conversationText = messages.map((m) => `${m.role}: ${m.content}`).join('\n');
  ...
  async classifyIntent(message: string, intents: string[]) {
    const response = await this.openai.chat.completions.create({
      model: 'gpt-4-turbo-preview',
      messages: [
        {
