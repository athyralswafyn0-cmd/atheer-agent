...
const client = axios.create({
  baseURL: 'https://atheer-agent-api.onrender.com/api/v1',
...
export const api = {
  ...
  updateBot: (id: string, data: Record<string, unknown>) => client.patch(`/bots/${id}`, data).then(r => r.data),
  deleteBot: (id: string) => client.delete(`/bots/${id}`).then(r => r.data),

  getConversations: (botId: string, params?: { page?: number; limit?: number; status?: string }) => client.get(`/conversations/bot/${botId}`, { params }).then(r => r.data),
  getConversation: (id: string) => client.get(`/conversations/${id}`).then(r => r.data),

  getLeads: (botId: string, params?: { page?: number; limit?: number; status?: string; search?: string }) => client.get(`/leads/bot/${botId}`, { params }).then(r => r.data),
  createLead: (data: { name: string; email?: string; phone?: string; company?: string; botId: string }) => client.post('/leads', data).then(r => r.data),

  getKnowledgeBases: (botId: string, page = 1, limit = 20) => client.get(`/knowledge/bot/${botId}`, { params: { page, limit } }).then(r => r.data),
  createKnowledgeBase: (botId: string, data: { name: string; type: string; source: string; metadata?: Record<string, unknown> }) => client.post(`/knowledge/bot/${botId}`, data).then(r => r.data),
  deleteKnowledgeBase: (id: string) => client.delete(`/knowledge/${id}`).then(r => r.data),

  getIntegrations: () => client.get('/integrations').then(r => r.data),
  createIntegration: (data: { name: string; type: string; config: Record<string, unknown> }) => client.post('/integrations', data).then(r => r.data),

  getAnalyticsOverview: (botId?: string) => client.get('/analytics/overview', { params: { botId } }).then(r => r.data),

  getEmbedConfig: (botId: string) => client.get(`/embed/bots/${botId}/config`).then(r => r.data),
  sendEmbedMessage: (botId: string, data: { message: string; conversationId?: string; sessionId?: string }) => client.post(`/embed/bots/${botId}/chat`, data).then(r => r.data),
  submitLead: (botId: string, data: { conversationId: string; name: string; email: string; phone?: string; company?: string; metadata?: Record<string, unknown> }) => client.post(`/embed/bots/${botId}/leads`, data).then(r => r.data),
  ...
  deletePartnerOrganization: (id: string) => client.delete(`/partners/organizations/${id}`).then(r => r.data),
