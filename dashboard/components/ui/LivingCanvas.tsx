...
export function LivingCanvas() {
  ...
  useEffect(() => {
    ...
    buffersRef.current = { position: posBuffer, index: idxBuffer };
    ...
    // Scroll tracking via IntersectionObserver on chapter sections
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const progress = parseFloat(entry.target.getAttribute('data-chapter-progress') || '0');
          scrollRef.current = progress;
        }
      });
    }, { threshold: [0, 0.5, 1] });

    chapterSections.forEach(el => observer.observe(el));
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      mouseRef.current.targetY = 1.0 - (touch.clientY - rect.top) / rect.height;
    };
    ...
    const animate = (time: number) => {
      ...
      // Get chapter state
      const state = getChapterState(scrollRef.current);

      gl.useProgram(program);
      gl.bindVertexArray(vao);

      gl.uniform1f(uniformsRef.current.u_time, time - startTimeRef.current);
      gl.uniform2f(
        uniformsRef.current.u_resolution,
        resolutionRef.current.width * resolutionRef.current.dpr,
        resolutionRef.current.height * resolutionRef.current.dpr
      );
      gl.uniform2f(uniformsRef.current.u_mouse, mouseRef.current.x, mouseRef.current.y);
      gl.uniform1f(uniformsRef.current.u_dpr, resolutionRef.current.dpr);

      // Palette
      gl.uniform3fv(uniformsRef.current.u_palette_bgBase, state.palette.bgBase);
      gl.uniform3fv(uniformsRef.current.u_palette_accent1, state.palette.accent1);
      gl.uniform3fv(uniformsRef.current.u_palette_accent2, state.palette.accent2);
      gl.uniform3fv(uniformsRef.current.u_palette_particleA, state.palette.particleA);
      gl.uniform3fv(uniformsRef.current.u_palette_particleB, state.palette.particleB);
      gl.uniform3fv(uniformsRef.current.u_palette_connection, state.palette.connection);

      // Chapter params
      gl.uniform1f(uniformsRef.current.u_camera, state.camera);
      gl.uniform1f(uniformsRef.current.u_density, state.density);
      gl.uniform1f(uniformsRef.current.u_connectivity, state.connectivity);
      gl.uniform1f(uniformsRef.current.u_flowSpeed, state.flowSpeed);
    ...
    return () => {
      ...
      if (buffersRef.current?.position) gl.deleteBuffer(buffersRef.current.position as WebGLBuffer);
      if (buffersRef.current?.index) gl.deleteBuffer(buffersRef.current.index as WebGLBuffer);
    };
  }, []);
